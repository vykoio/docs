...
### <Pill type="GET">GET</Pill> /:id/:email_count?
...
| ----------- | -------- | -------------------------------------------------- |
| id          | Yes      | ID of inbox.                                       |
| email_count | No       | Amount of messages to be returned. Defaults to 50. |
...
[
  {
    ...
    "to_email": "receive@vyko.email", //Recipient email
    "to_name": "Vyko Receiver", //Receipient display name
  ...
  },
  {
    ...
    "to_email": "receive@vyko.email",
    "to_name": "Vyko Receiver",
...
Deletes the entire inbox and emails along with removing all members (as it no longer exists). If you want to remove all emails but keep the inbox and it's members, see [DELETE /email/:inbox_id/:id?](#delete-emailinbox_idid)
...
### <Pill type="GET">GET</Pill> /email/:id
...
### <Pill type="DELETE">DELETE</Pill> /:inbox_id/email/:id?
...
| Parameter | Required | Description                                                                                                   |
| --------- | -------- | ------------------------------------------------------------------------------------------------------------- |
| inbox_id  | Yes      | ID of inbox.                                                                                                  |
| id        | No       | ID of email you want removed. **If not provided, _all emails_ within the provided inbox ID will be removed**. |
...
### <Pill type="POST">GET</Pill> /:inbox_id/users
...
### <Pill type="POST">POST</Pill> /users/invite
...
### <Pill type="PATCH">PATCH</Pill> /users/role
...
### <Pill type="GET">GET</Pill> /:inbox_id/users/leave/:id?
...
| --------- | -------- | ----------------------------------------------------------------------------------------------------------------- |
| inbox_id  | Yes      | ID of inbox.                                                                                                      |
| id        | No       | ID of user that should be removed. If this is not provided, the requester will be removed from the inbox instead. |
...
| 1    | Member        | User is able to see messages.                                                                                |
