...
Deletes the entire inbox and emails along with removing all members (as it no longer exists). If you want to remove all emails but keep the inbox and it's members, see [DELETE /email/:id/:email_id?](#delete-idemailemail_id)
